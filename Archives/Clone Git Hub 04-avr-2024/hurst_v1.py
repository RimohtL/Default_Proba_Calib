...
def BSM_H(ticker, market_cap, debt, T=1, frequency=[36,252], rf=0, epsilon=10e-3,H0=0.5):
    ...
    def d1(x, sigma_A, t, T, H):
        return ( (np.log(x / company_debt)) + rf*(T-t) + 0.5 * sigma_A ** 2*(T**(2*H)-t**(2*H) )/ (
        sigma_A*np.sqrt(T**(2*H)-t**(2*H))  ) )

    def d2(x, sigma_A, t, T, H):
        return ((np.log(x / company_debt)) + rf*(T-t) - 0.5 * sigma_A ** 2*(T**(2*H)-t**(2*H) )/ (
        sigma_A*np.sqrt(T**(2*H)-t**(2*H))  ))

    # inverse the black scholes formula
    def merton_formula(x, rf, t ,T, H):
        d1_term = x * norm.cdf(d1(x, sigma_A, t,T, H))
        d2_term = company_debt * np.exp(-rf * (T-t)) * norm.cdf(d2(x, sigma_A, t,T, H))
    ...
    while np.abs(sigma_A - sigma_A_former) > epsilon:
        ...
        for f in frequency:
            ...
            for day in days:
                ...
                fasset_values.append(optimize.newton(merton_formula, company_debt, args=(rf,t,1+T,H),maxiter=100))
    ...
    distance_to_default = d2(asset_values[frequency[0]][-1], sigma_A, 0, T,H)
